    ...
    while (true) {
        ...
        if (number_of_descriptors_with_event > 0) {
            ...
            for (auto descriptor : descriptors) {
                if (descriptor.revents & POLLIN) { //NOLINT
                    if (descriptor.fd == server_descriptor.fd) {
                        tcp_server->accept();
                    } else {
                        std::string request_buffer;

                        while (descriptor.revents & POLLIN) { //NOLINT
                            auto receive_data = Server::receive_data(descriptor.fd);
                        ...
                        if (!request_buffer.empty()) {
                            ...
                            if (response.m_should_respond) {
                                ...
                                Server::send_data(descriptor.fd, response.m_response.c_str());
                            }
